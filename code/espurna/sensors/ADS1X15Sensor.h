...
class ADS1X15Sensor : public I2CSensor {
    ...
    public:
        ...
        String description() {
            char buffer[30];
            snprintf(buffer, sizeof(buffer), "ADS1%d15 @ I2C (0x%02X)", _type == ADS1X15_CHIP_ADS1015 ? 0 : 1, _address);
        ...
        String slot(unsigned char index) {
            ...
            snprintf(buffer, sizeof(buffer), "ADS1%d15 (A%d) @ I2C (0x%02X)", _type == ADS1X15_CHIP_ADS1015 ? 0 : 1, channel, _address);
            return String(buffer);
        }
